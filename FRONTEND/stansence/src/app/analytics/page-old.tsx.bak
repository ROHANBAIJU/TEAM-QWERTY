...
export default function Analytics() {
  ...
  // Update symptom data from real WebSocket or demo mode (throttled to prevent excessive updates)
  useEffect(() => {
    if (latestData && !demoMode && latestData.scores) {
      const now = Date.now();
      // Only update if 3 seconds have passed since last update
      if (now - lastUpdateTime >= 3000) {
        // Use real AI scores from backend (already 0-1, convert to 0-100)
        const tremor = latestData.scores.tremor * 100;
        const rigidity = latestData.scores.rigidity * 100;
        const slowness = latestData.scores.slowness * 100;
        const gait = latestData.scores.gait * 100;

        console.log('Analytics updating with:', { tremor, rigidity, gait, slowness });
        console.log('Analysis:', latestData.analysis);
        ...
        setSymptomData((prev) => ({
          ...
          gait,
          history: {
            tremor: [...prev.history.tremor.slice(-8), tremor],
            rigidity: [...prev.history.rigidity.slice(-8), rigidity],
            slowness: [...prev.history.slowness.slice(-8), slowness],
            gait: [...prev.history.gait.slice(-8), gait],
          },
        }));
  ...
  }, [latestData, demoMode, lastUpdateTime]);
          ...
          <div className="analytics-core-card">
            <div className="analytics-core-header">
              ...
              <div className="analytics-core-actions">
                ...
                <div
                  id="sensor-status"
                  className={`sensor-status ${demoMode ? "demo" : isConnected ? "connected" : "disconnected"}`}
                >
                  {demoMode ? "📡 Demo" : isConnected ? "🔌 Live Data" : "⚠️ Disconnected"}
                  {latestData && !demoMode && (
                    <span style={{ marginLeft: '8px', fontSize: '11px', opacity: 0.7 }}>
                      {latestData.scores ? '✓ AI' : '○ No AI'}
                    </span>
                  )}
                </div>
                <button
