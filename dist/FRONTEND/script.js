...
function initializeCharts() {
    ...
    const symptomCtx = (_a = document.getElementById('symptomChart')) === null || _a === void 0 ? void 0 : _a.getContext('2d');
    if (symptomCtx) {
    ...
    const fallCtx = (_b = document.getElementById('fallChart')) === null || _b === void 0 ? void 0 : _b.getContext('2d');
    if (fallCtx) {
        const fallChartData = {
...
function markDoseAsTaken(doseTime) {
    ...
    doseItems.forEach((item) => {
        ...
        if (timeElement && timeElement.textContent === doseTime) {
            ...
            let statusBadge = item.querySelector('.dose-status');
            if (!statusBadge) {
...
if (logSymptomButton) {
    logSymptomButton.addEventListener('click', () => {
        ...
        const patientNotesTab = document.querySelector('.menu-item[data-tab="patient-notes"]');
        if (patientNotesTab) {
            patientNotesTab.click();
            // After tab switch, focus on textarea
            setTimeout(() => {
                const notesTextarea = document.querySelector('#noteTextarea');
                if (notesTextarea) {
                    notesTextarea.focus();
